...
<学习资料>{json.dumps(section_payload, ensure_ascii=False)}</学习资料>"""
    ...
    async def merge_knowledge_points(
        ...
        if len(candidates) > 24:
            merged: list[KnowledgePointCandidate] = []
            for offset in range(0, len(candidates), 24):
                merged_map = await self.merge_knowledge_points(candidates[offset : offset + 24])
                merged.extend(merged_map.knowledge_points)
            if len(merged) > 24:
                return await self.merge_knowledge_points(merged)
            return KnowledgePointMap(knowledge_points=self._dedupe_points(merged))
        schema = KnowledgePointMap.model_json_schema()
...
<候选知识点>{json.dumps(candidate_payload, ensure_ascii=False)}</候选知识点>"""
    ...
    def _parse_model(payload: str, model: type[BaseModel], label: str):
        ...
        try:
            ...
            raise RuntimeError(f"DeepSeek 返回的{label}结构无效：{detail}") from exc
