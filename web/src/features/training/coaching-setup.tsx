...
export function CoachingSetup({ mode, initialFocus = "", initialDifficulty = "guided" }: { mode: CoachingMode; initialFocus?: string; initialDifficulty?: CoachingDifficulty }) {
  ...
  async function create(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/coaching-sessions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode, channel, target_role: role, training_goal: goal, source_ids: [], exercise_type: exercise, difficulty }) });
