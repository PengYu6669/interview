  ...
  return <main className="content-container profile-page">
    ...
    {error ? <section className="matrix-empty" role="alert"><Target size={21} /><span>{error}</span></section> : profile && <>
      ...
      <section className="ability-matrix-section coaching-ability-section">
        <div className="ability-matrix-header"><div><h2>专项训练能力</h2><p>{profile.coaching.session_count ? `共 ${profile.coaching.session_count} 次训练，${profile.coaching.completed_count} 次完成；只聚合有原句证据的观察` : "完成结构化表达或业务 Sense 训练后，这里会形成独立能力观察"}</p></div>{profile.coaching.next_mode && <Link href={`/training/new?mode=${profile.coaching.next_mode}`} className="secondary-button"><Target size={14} />练习最弱项</Link>}</div>
        {profile.coaching.skills.length ? <div className="matrix-grid">{profile.coaching.skills.map((item) => <article key={item.dimension}><span className="matrix-score">{item.score}</span><div className="skill-summary"><div><h3>{COACHING_DIMENSION_LABELS[item.dimension] ?? item.dimension}</h3><span className="confidence-label">{COACHING_MODE_LABELS[item.mode]}</span></div><p>{item.evidence_count} 条回答证据 · {item.session_count} 次训练 · 可信度 {Math.round(item.confidence * 100)}%</p><small>{item.latest_feedback}</small></div><span className="matrix-trend flat">{item.score >= 80 ? "表现稳定" : item.score >= 60 ? "继续巩固" : "优先训练"}</span><div className="skill-actions"><Link href={`/training/${item.source_session_id}`} title="查看来源训练" aria-label={`查看 ${COACHING_DIMENSION_LABELS[item.dimension] ?? item.dimension} 的来源训练`}><FileChartColumn size={14} /></Link><Link href={`/training/new?mode=${item.mode}`} className="coaching-retrain"><Target size={13} />再练一次</Link></div></article>)}</div> : <div className="matrix-empty"><Target size={21} /><span>{profile.coaching.session_count ? "当前训练还没有足够的有效回答证据" : "还没有专项训练记录"}</span><Link href="/training">进入训练中心 <ArrowRight size={14} /></Link></div>}
      </section>
