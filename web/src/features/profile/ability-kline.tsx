...
export function AbilityKline({ points }: { points: AbilityKlinePoint[] }) {
  ...
  return <><TrendLine points={points.map((point) => ({ label: point.date, value: point.close, confidence: point.confidence }))} /><div className="ability-session-list">{points.map((point) => <a key={point.session_id} href={`/report?session=${point.session_id}`} aria-label={`${point.date}，当场表现 ${point.close} 分，查看对应报告`}>
