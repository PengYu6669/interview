...
export function AbilityKline({ points }: { points: AbilityKlinePoint[] }) {
  if (!points.length) return <div className="grid min-h-36 place-content-center gap-1 text-center" role="status"><strong className="text-sm">暂无训练表现记录</strong><span className="text-xs text-[var(--muted)]">完成模拟面试并生成复盘后出现</span></div>;
