...
export function InterviewReport({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/interview-sessions/${encodeURIComponent(sourceId)}/report`, { cache: "no-store" })
      .then(async (response) => {
        const payload: unknown = await response.json();
        if (!response.ok) throw new Error(errorMessage(payload, "上次复盘读取失败"));
        if (active) setSourceReport(interviewReportSchema.parse(payload));
      })
      .catch((caught) => {
        if (active) setSourceReportError({ sourceId, message: caught instanceof Error ? caught.message : "上次复盘读取失败" });
      });
    return () => { active = false; };
  ...
  return <main className="content-container report-page">
    <PageIntro
      eyebrow={`${partial ? "中途结束" : "训练完成"} · 回答覆盖 ${report.content.evidence_coverage}%`}
      title="本次面试报告"
      description={`${report.target_role} · ${report.duration_minutes} 分钟 · ${report.turn_count} 轮回答。评分只覆盖本场实际出现的回答。`}
    ...
    {partial && <div className="report-limitation"><AlertTriangle size={17} /><p><strong>阶段性报告</strong>这场面试中途结束，分数和建议不能代表完整岗位能力，请结合回答覆盖和置信度阅读。</p></div>}
    ...
    <section className="report-overview">
      ...
      <div className="score-breakdown"><div className="panel-heading"><div><span>能力分布</span><small>每项均对应本场回答，可申请独立复核</small></div></div>{report.content.skill_scores.length >= 3 && <RadarChart data={report.content.skill_scores.map((item) => ({ label: item.skill, value: item.score }))} />}{report.content.skill_scores.map((item, index) => {
        ...
        return <div className="score-row" key={`${item.skill}-${index}`}><div><span>{item.skill}</span><strong>{displayedScore}{displayedScore !== item.score && <small>原 {item.score}</small>}</strong></div><div className="score-track"><i style={{ width: `${displayedScore}%` }} /></div><div className="score-evidence-line"><small>第 {item.evidence_turns.join("、")} 轮回答 · 置信度 {Math.round(item.confidence * 100)}%</small><button type="button" onClick={() => openReview({ skillIndex: index, skill: item.skill, originalScore: item.score })}><Scale size={13} />{review ? "再次处理" : "对此评分有异议"}</button></div>{review && <ReviewResult review={review} />}</div>;
      })}</div>
      <div className="report-strength"><CheckCircle2 size={21} /><div><span>表现优势</span><strong>{report.content.strengths.length ? `${report.content.strengths.length} 项表现较稳定` : "暂未确认明显优势"}</strong><p>{report.content.strengths.length ? report.content.strengths.map((item) => item.skill).join(" · ") : "完成更多问题后，系统才能形成更稳定的优势判断。"}</p></div></div>
    </section>

    <section className="report-evidence-section">
      <div className="section-title"><div><h2>表现优势</h2><p>本场回答中较稳定的正向表现</p></div><StatusBadge tone="success">{report.content.strengths.length} 项</StatusBadge></div>
      {report.content.strengths.length ? <div className="report-strength-list">{report.content.strengths.map((item, index) => <EvidenceChain key={`${item.skill}-${index}`} conclusion={item.title} evidence={item.evidence_quote} basis={item.analysis} confidence={report.content.skill_scores.find((score) => score.skill === item.skill)?.confidence} action="在更高难度或新的追问情境中再次验证这一表现。" meta={`${item.skill} · 第 ${item.evidence_turns.join("、")} 轮`} tone="positive" />)}</div> : <div className="report-empty-evidence"><CheckCircle2 size={18} /><span>当前回答还不足以形成稳定优势判断</span></div>}
    </section>
    ...
    {(report.verification_status === "degraded" || report.verified_claims.length > 0) && <section className="report-verification-section"><div className="section-title"><div><h2>技术表达检查</h2><p>查看本场回答中的技术结论</p></div><StatusBadge tone={report.verification_status === "degraded" ? "warning" : "success"}>{report.verification_status === "degraded" ? "暂不可用" : `${report.verified_claims.length} 条`}</StatusBadge></div>{report.verification_status === "degraded" ? <div className="verification-degraded"><AlertTriangle size={17} /><p><strong>本次技术检查未完成</strong>{report.verification_error || "技术检查暂时不可用，本场不会据此判断回答错误。"}</p></div> : <div className="verification-list">{report.verified_claims.map((claim, index) => <VerifiedClaimItem key={`${claim.sequence}-${index}`} claim={claim} />)}</div>}</section>}

    <div className="report-content">
      <section className="evidence-list"><div className="section-title"><div><h2>优先改进项</h2><p>选择要带入下一场弱项复训的内容</p></div><StatusBadge>{selectedImprovements.length} / {report.content.improvements.length} 已选</StatusBadge></div>{report.content.improvements.length ? report.content.improvements.map((item, index) => <EvidenceItem key={`${item.skill}-${index}`} item={item} confidence={report.content.skill_scores.find((score) => score.skill === item.skill)?.confidence} selected={selectedImprovements.includes(index)} onToggle={() => toggleImprovement(index)} />) : <div className="evidence-item"><h3>当前回答还不足以形成改进项</h3><p>继续完成更多面试问题后再生成完整报告。</p></div>}</section>
    ...
    <Dialog open={Boolean(reviewTarget)} onOpenChange={(open) => { if (!open && !reviewSubmitting) setReviewTarget(null); }}>
      {reviewTarget && <DialogContent aria-describedby="review-dialog-description">
        ...
        <DialogDescription id="review-dialog-description">原报告和回答会保留，处理结果将作为单独记录附在报告上。</DialogDescription>
...
function RetrainingComparison({
  ...
  if (error) return <section className="report-retraining-comparison"><div className="section-title"><div><h2>弱项复训验证</h2><p>上次复盘暂时无法读取</p></div></div><div className="report-limitation"><AlertTriangle size={17} /><p>{error}</p></div></section>;
  if (!sourceReport) return <section className="report-retraining-comparison"><AiWorkReceipt title="正在读取上次训练" description="系统正在恢复上一场改进项用于前后核对。" activeStep={0} steps={[{ label: "正在读取已保存的复盘", detail: "仅比较两次训练表现，不会重新评分" }]} footer="上次复盘不可用时会明确提示。" /></section>;
  ...
  const comparisons = sourceReport.content.improvements.map((previous) => {
    ...
    if (strength) return { previous, current, score, title: "本场表现有所改善", basis: `上次复盘将“${previous.title}”列为改进项，本场同一能力表现较稳定。`, tone: "positive" as const, action: "在新的题目或更高压力下再次验证，确认表现是否稳定。" };
    if (improvement) return { previous, current, score, title: "本场仍需继续改进", basis: `两次训练都在“${previous.skill}”下发现相似问题，当前还不能判定已经改善。`, tone: "warning" as const, action: improvement.improvement ?? report.content.next_training };
    return { previous, current: null, score, title: "本场未覆盖同一能力", basis: `上次复盘中的“${previous.skill}”没有在本场优势或改进项中出现，暂时无法判断变化。`, tone: "neutral" as const, action: "安排一次明确考察该能力的训练，形成可比较结果。" };
  });

  return <section className="report-retraining-comparison">
    <div className="section-title"><div><h2>弱项复训验证</h2><p>对比上次训练与本场表现</p></div><Link href={`/report?session=${sourceReport.session_id}`} className="secondary-button">查看上次复盘 <ArrowLeft size={14} /></Link></div>
    <div className="report-key-finding-list">{comparisons.map(({ previous, current, score, title, basis, tone, action }) => <EvidenceChain key={`${previous.skill}-${previous.title}`} conclusion={title} previousEvidence={previous.evidence_quote} evidence={current?.evidence_quote} basis={basis} confidence={score?.confidence} action={action} meta={`${previous.skill} · 上次改进项`} tone={tone} />)}</div>
  </section>;
}
...
function VerifiedClaimItem({
  ...
  return <article className={`verification-item ${claim.result}`}><div className="verification-item-heading"><BookOpenCheck size={17} /><div><span>第 {claim.sequence} 轮技术表述</span><h3>{claim.claim}</h3></div><StatusBadge tone={tone}>{labels[claim.result]} · {Math.round(claim.confidence * 100)}%</StatusBadge></div><blockquote><span>你的原话</span>“{claim.evidence_quote}”</blockquote><p>{claim.rationale}</p></article>;
}
...
function ReportProcessState({
  ...
  return <main className="content-container report-process-page"><AiWorkReceipt title={generating ? "正在生成本次面试报告" : "正在确认报告状态"} description={message} activeStep={0} steps={[{ label: generating ? "报告任务正在后台运行" : "正在读取已保存的报告状态", detail: "完成后会保留回答、评分版本和置信度" }]} footer={generating ? `已等待 ${elapsedSeconds} 秒，离开页面后任务仍可从训练记录恢复。` : "只读取已保存的训练状态，不会重复生成报告。"} /></main>;
}
