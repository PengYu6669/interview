...
export function StructuredProfile({
  ...
  return (
    <div className="review-fields">
        ...
        <div className="review-block-content">
          {skills.length ? skills.map((skill) => (
            <div className="editable-row" key={`${skill.value}-${skill.evidence}`}>
              <div><input className="editable-inline-input" aria-label="技能名称" value={skill.value} onChange={(event) => onChange({ ...result, profile: { ...profile, skills: skills.map((item) => item === skill ? { ...item, value: event.target.value } : item) } })} /><p>原文证据：{skill.evidence}</p></div>
      ...
      <section className="review-block">
        <div className="review-block-title"><div><h2>项目经历</h2><p>不显示模型无法引用原文的项目</p></div></div>
        <div className="review-block-content">
          {projects.length ? projects.map((project) => (
            <article className="editable-row" key={`${project.name}-${project.evidence}`}>
              <div><input className="editable-inline-input" aria-label="项目名称" value={project.name} onChange={(event) => onChange({ ...result, profile: { ...profile, projects: projects.map((item) => item === project ? { ...item, name: event.target.value } : item) } })} /><textarea className="editable-inline-textarea" aria-label={`${project.name} 项目描述`} value={project.description} onChange={(event) => onChange({ ...result, profile: { ...profile, projects: projects.map((item) => item === project ? { ...item, description: event.target.value } : item) } })} /><p>原文证据：{project.evidence}</p></div><button type="button" className="icon-action" aria-label={`删除 ${project.name}`} onClick={() => onChange({ ...result, profile: { ...profile, projects: projects.filter((item) => item !== project) } })}><Trash2 size={15} /></button>
            </article>
          )) : <p className="text-xs text-[var(--muted)]">没有提取到证据充分的项目经历。</p>}
        </div>
      </section>
