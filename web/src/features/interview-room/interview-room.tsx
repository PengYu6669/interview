...
import { AlertTriangle, ArrowLeft, AudioLines, Camera, Captions, CheckCircle2, Clock3, LoaderCircle, Mic, MicOff, Pause, PhoneOff, Play, RefreshCw, Send, Settings, Square, Video, VideoOff, Volume2, X } from "lucide-react";
...
export function InterviewRoom({ sessionId }: { sessionId: string }) {
  ...
  async function startInterview() {
    ...
    try {
      ...
      if (continuousVoiceRef.current) await startVoiceAnswer(startedRuntime);
    } catch (caught) {
  ...
  async function submitAnswer(answerText = answer, mode = answerMode, targetRuntime = runtime) {
    ...
    try {
      ...
      if (mode === "voice" && nextRuntime.status === "started" && nextRuntime.current_question) {
        await playQuestion(spokenQuestion(nextRuntime));
        if (continuousVoiceRef.current) await startVoiceAnswer(nextRuntime, true);
      } else {
        setVoiceStatus("idle");
      }
      setConnectionState("online");
  ...
  if (runtime.status === "completed" || runtime.status === "ended") return <div className="interview-entry-state complete"><CheckCircle2 size={30} /><strong>{runtime.status === "completed" ? "本场面试已完成并保存" : "本场面试已结束并保存"}</strong><p>{runtime.status === "completed" ? `本场 ${runtime.answered_questions} 道主问题和全部追问回答已写入训练记录。` : `面试进度停在第 ${Math.min(runtime.answered_questions + 1, runtime.total_questions)} 题，已提交的回答均已写入训练记录。`}</p><span className="interview-save-note">复盘报告尚未生成，点击下方按钮后才会调用 DeepSeek。</span>{runtime.status === "completed" && (runtime.interviewer_reply || runtime.closing_statement) && <blockquote className="interview-closing-statement">{[runtime.interviewer_reply, runtime.closing_statement].filter(Boolean).join(" ")}</blockquote>}<div className="interview-complete-actions"><Link href={`/report?session=${runtime.id}`} className="primary-cta">生成复盘报告</Link><Link href="/history" className="secondary-button">查看已保存记录</Link></div></div>;
  ...
  return <div className="meeting-app feishu-room">
    <header className="meeting-header"><div className="meeting-title"><span className="meeting-logo">面</span><div><h1>{runtime.target_company ? `${runtime.target_company} · ` : ""}{runtime.target_role}</h1><p><span className={`live-dot ${connectionReady ? "" : "disconnected"}`} />{runtime.status === "paused" ? "面试已暂停" : connectionReady ? "面试进行中" : "连接待恢复"} · {sessionContext.round} · {sessionContext.type}</p></div></div><div className="meeting-meta"><span><Clock3 size={15} />{formatDuration(remainingSeconds)}</span><button type="button" title="面试设置" aria-label="面试设置" aria-expanded={settingsOpen} onClick={() => setSettingsOpen((value) => !value)}><Settings size={18} /></button></div></header>
    <ConnectionBanner state={connectionState} hasDraft={Boolean(answer.trim())} onRetry={() => void recoverConnection()} />
    <main className={`meeting-main ${textPanel ? "with-answer-panel" : ""}`}>
      <section className="video-stage">
        <div className="interviewer-tile"><div className="participant-label"><span className="speaking-dot" />AI 面试官</div><div className="interviewer-presence"><div className="interviewer-avatar">林</div><strong>林老师</strong><span>AI 技术面试官</span><div className={`sound-wave ${speaking ? "is-speaking" : ""}`}>{[1, 2, 3, 4, 5].map((item) => <i key={item} />)}</div></div></div>
        ...
        <div className={`self-video ${cameraOn ? "camera-active" : ""}`}>{cameraOn ? <video ref={videoRef} autoPlay muted playsInline /> : <><div className="self-avatar">你</div><span>摄像头已关闭</span></>}<small>{muted && <MicOff size={11} />} 你</small></div>
      </section>
      ...
      <aside className="meeting-sidebar"><div className="sidebar-title"><div><strong>面试进度</strong><span>{runtime.answered_questions} / {runtime.total_questions} 已完成</span></div></div><div className="phase-timeline">{runtime.phases.map((phase, index) => { const status = index < runtime.current_phase_index ? "done" : index === runtime.current_phase_index ? "active" : "pending"; return <div className={`timeline-phase ${status}`} key={phase.name}><span>{status === "done" ? "✓" : index + 1}</span><div><strong>{phase.name}</strong><small>{status === "active" ? "正在进行" : status === "done" ? "已完成" : `${phase.minutes} 分钟`}</small><p>{phase.skills.slice(0, 3).join(" · ")}</p></div></div>; })}</div><div className="sidebar-notes"><Captions size={16} /><div><strong>{captionsOn ? "问题字幕已开启" : "问题字幕已关闭"}</strong><span>回答会在提交后保存为面试证据</span></div></div></aside></main>
    <footer className="meeting-controls"><div className="control-hint"><span className={`live-dot ${connectionReady ? "" : "disconnected"}`} />{!connectionReady ? "连接恢复前不会提交回答" : runtime.status === "paused" ? "计时与回答已暂停" : voiceStatus === "thinking" ? "面试官正在分析你的回答" : recording ? "直接说话，持续停顿约 3 秒后自动提交" : transcribing ? "正在整理识别结果" : autoVoice ? "自动语音面试已就绪" : "手动语音模式"}</div><div className="control-group"><RoomControl active={!muted} label={muted ? "取消静音" : "静音"} onClick={toggleMic} icon={muted ? <MicOff /> : <Mic />} /><RoomControl active={cameraOn} label={cameraOn ? "关闭摄像头" : "开启摄像头"} onClick={toggleCamera} icon={cameraOn ? <Video /> : <VideoOff />} /><RoomControl active={runtime.status === "paused"} disabled={!connectionReady || submittingAnswer || transcribing} label={runtime.status === "paused" ? "继续面试" : "暂停面试"} onClick={() => void changeSessionState(runtime.status === "paused" ? "resume" : "pause")} icon={runtime.status === "paused" ? <Play /> : <Pause />} /><RoomControl active={recording} disabled={!connectionReady || submittingAnswer || speaking || submissionUncertain} label={recording ? "结束回答" : submittingAnswer || speaking ? "请稍候" : submissionUncertain ? "先确认提交" : "开始回答"} onClick={() => { if (runtime.status === "paused" || submittingAnswer || speaking || submissionUncertain) return; if (recording) void stopVoiceAnswer(); else { continuousVoiceRef.current = autoVoice; void startVoiceAnswer(runtime); } }} icon={recording ? <Square /> : submittingAnswer ? <LoaderCircle className="spin" /> : <AudioLines />} /><RoomControl active={textPanel && answerMode === "text"} label="文字备用" onClick={() => { if (runtime.status === "paused" || recording || transcribing || submittingAnswer || submissionUncertain) return; continuousVoiceRef.current = false; setAutoVoice(false); setAnswerMode("text"); if (!answerRef.current.trim()) clientMessageIdRef.current = null; setTextPanel((value) => answerMode === "text" ? !value : true); }} icon={<Captions />} /><button className="hangup-button" type="button" title="结束面试" aria-label="结束面试" disabled={!connectionReady || submittingAnswer || transcribing} onClick={() => setEndConfirmation(true)}><PhoneOff size={20} /><span>结束</span></button></div><div className="control-mode"><span>问题 {Math.min(runtime.answered_questions + 1, runtime.total_questions)} / {runtime.total_questions}</span></div></footer>
