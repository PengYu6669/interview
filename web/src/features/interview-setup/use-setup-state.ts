...
import { calculateReadiness, DocumentParseStatus, readSetupState, serializeSetupState, SETUP_STATE_STORAGE_KEY, SetupState } from "./model";

export function useSetupState(careerPlanItemId?: string) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (!response.ok) {
        const detail = typeof payload === "object" && payload !== null && "detail" in payload ? String(payload.detail) : "简历解析失败";
  ...
  async function continueToReview() {
    ...
    try {
      ...
      const payload = {
        resume_filename: parsedDocument.filename, resume_text: parsedDocument.text, jd: state.jd,
        target_role: state.role, target_company: state.company, target_level: state.level,
        interview_round: state.interviewRound, interview_type: state.interviewType,
        mode: state.mode, duration_minutes: state.duration, pressure_level: state.pressure,
        depth_level: state.depth, guidance_level: state.guidance,
        question_ids: state.selectedQuestions.map((item) => item.id),
        training_focus: state.trainingFocus, source_session_id: sourceSessionId,
        career_plan_item_id: linkedPlanItemId,
      };
      ...
      if (response.ok) {
        ...
        const detail = typeof errorPayload === "object" && errorPayload && "detail" in errorPayload ? String(errorPayload.detail) : "训练草稿保存失败";
        throw new Error(detail);
      }
