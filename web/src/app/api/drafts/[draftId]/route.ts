...
const schema = z.object({ resume_filename: z.string().max(255).optional(), resume_text: z.string().min(1).max(80_000).optional(), jd: z.string().max(30_000).optional(), target_role: z.string().min(1).max(150).optional(), target_company: z.string().max(100).optional(), target_level: z.enum(["intern", "campus", "mid", "senior"]).optional(), interview_round: z.enum(["first", "second", "final", "manager"]).optional(), interview_type: z.enum(["comprehensive", "project", "technical", "system_design", "behavioral", "weak_area"]).optional(), mode: z.string().max(30).optional(), duration_minutes: z.number().int().min(1).max(180).optional(), pressure_level: z.number().int().min(1).max(5).optional(), depth_level: z.number().int().min(1).max(5).optional(), guidance_level: z.number().int().min(1).max(5).optional(), question_ids: z.array(z.string().uuid()).max(20).optional(), training_focus: z.string().max(500).optional(), extraction: z.record(z.string(), z.unknown()).nullable().optional() });
