...
export default async function HistoryPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  return <PageShell active="history">
    <div className="content-container"><GrowthTabs active={view} /></div>
    {view === "capabilities" ? <AbilityProfile /> : <main className="content-container history-page">
      <PageIntro eyebrow={view === "plan" ? "长期求职" : "训练记录"} title={view === "plan" ? "把长期目标拆成本周可完成的行动" : "每一次训练，都应该留下可复查的证据"} description={view === "plan" ? "求职画像只保存你确认的内容，周计划可以随现实进度持续调整。" : "按时间回看面试题目、回答证据、评分版本和改进建议。"} actions={view === "records" ? <Link href="/setup" className="primary-cta">开始新训练 <ArrowRight size={16} /></Link> : undefined} />
